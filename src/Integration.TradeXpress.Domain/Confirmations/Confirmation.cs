...
/// <summary>
/// <b>Teyit</b> — organizasyon-içi (kasa↔kasa) bir process'in <b>karşılıklı mirror onayı</b>. company-scoped,
...
/// (<see cref="Declare"/> — sistem karşı satırı yazmaz), gönderen teyit eder (<see cref="Confirm"/>) → ancak o an
/// birbirinin mirror'ı olan iki bacak (gönderen −, alıcı +) atomik postlanır.
///
...
/// (emtia/varyant/miktar/tutar/birimler/yön) payload'un <b>denormalize MIRROR ANAHTARI</b>dır — sorgu/grid ve
/// mirror karşılaştırması için (<see cref="ToMirrorKey"/>).</para>
///
/// <para><b>Zero-trust:</b> tek taraflı beyan ötekinin defterini kımıldatmaz; SİSTEM karşı kaydı otomatik
/// üretmez — her taraf kendi gerçeğini kendi eliyle yazar. Yalnız çift-teyitli mirror çift postlanır. Değer,
/// teyit kapanana dek gönderenin sorumluluğundadır. Mirror kriteri (<see cref="ConfirmationMirrorKey"/> — TAM
/// mirror: emtia/varyant/miktar/tutar/ana+karşılık birimi, ZIT yön) AppService'te doğrulanır — tutmazsa fark
...
/// artık ALICININDIR — kendi eliyle yazdığı için sonradan inkâr edemez. Sistemin karşı satırı kendiliğinden
/// yazmamasının asıl sebebi de budur: kimse kimsenin kaydını onun yerine yazamaz, herkes kendi gerçeğini sahiplenir.</para>
///
...
public class Confirmation : FullAuditedAggregateRoot<Guid>, IMultiTenant, ICompanyOwned
{
    ...
    // ── Mirror anahtarı: payload'un denormalize ölçütleri (spec §3) — ToMirrorKey ile okunur ──
    ...
    /// <summary>Gönderenin yönü; mirror'ı AYNI EKSENDE zıt yöndür (Giriş↔Çıkış, Alış↔Satış…).</summary>
    public virtual ProcessDirectionType Direction { get; protected set; }

    /// <summary>Mirror kriteri emtiası (Nakit/Maden/Mamül…) — id-only; emtiasız tiplerde null.</summary>
    public virtual Guid? CommodityId { get; protected set; }

    /// <summary>Mirror kriteri varyantı (çok-varyantlı emtiada) — id-only; varyantsızda null.</summary>
    public virtual Guid? VariantId { get; protected set; }

    /// <summary>Mirror kriteri miktarı (adet/gram). Tipe göre 0 olabilir (ör. Nakit).</summary>
    public virtual decimal Quantity { get; protected set; }

    /// <summary>Mirror kriteri tutarı. Tipe göre 0 olabilir (ör. yalnız-adet satırı).</summary>
    public virtual decimal Amount { get; protected set; }

    /// <summary>Mirror kriteri ANA birimi (<c>VoucherLine.MainUnitId</c> karşılığı) — id-only referans.
    ...
    /// <summary>Mirror kriteri KARŞILIK birimi (<c>VoucherLine.PayUnitId</c>) — id-only; karşılıksızda null.</summary>
    public virtual Guid? PayUnitId { get; protected set; }

    /// <summary>Mirror kriteri karşılık tutarı (<c>VoucherLine.PayTotal</c>). Değerlemesiz teslimde 0.</summary>
    public virtual decimal PayTotal { get; protected set; }

    // ── Payload'lar: her taraf KENDİ satırını yazar (sistem karşı satırı yazmaz) ──
    ...
    /// kadar null (sistem KENDİLİĞİNDEN DOLDURMAZ; gönderenin payload'undan TÜRETİLMEZ).</summary>
    ...
    /// <summary>Bu Teyit'in (başlatan bacağının) mirror anahtarı — alıcının beyanı bununla kıyaslanır.
    ...
    /// <summary>Teklif → Beyan edildi. Alıcı KENDİ ELİYLE kendi girişini oluşturdu (sistem karşı satırı yazmaz) ve o satır
    /// payload olarak saklanır; gönderenin teyidi beklenir. Postlama YOK. Alıcının kaydının gönderenin kaydıyla
    /// MIRROR olduğu (<see cref="ConfirmationMirrorKey"/>) AppService'te doğrulanmış olmalı — tutmazsa buraya
    /// gelinmez.</summary>
    ...
    /// <summary>Beyan edildi → Teyitlendi. Gönderen alıcının mirror kaydını teyit eder; HER İKİ bacak (gönderen −,
