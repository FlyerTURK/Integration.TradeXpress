...
/// <summary>
/// Etsy kanal-ürününde bir VARYANTIN <b>kanal-özel reçete satırı</b> — <see cref="ProductVariantRecipeLine"/>'ın
/// (ürün-geneli reçete) kanal-scope KLONU. İlk açılışta ERP varyant reçetesinden kopyalanır, sonra BAĞIMSIZ
/// düzenlenir: kanal maliyeti + marj → Etsy türetilmiş fiyatı. <b>Company-owned</b> (<see cref="CompanyId"/>
/// kanal-üründen denormalize) + per-tenant. Anchor: <see cref="SalesChannelEtsyProductId"/> + <see cref="StockItemId"/>
/// (set-once) — <see cref="StockItemId"/> <see cref="SalesChannelEtsyProductStockItem"/>'ın KENDİ Id'sidir (ERP
/// ProductVariantId DEĞİL — böylece ERP-backed VE Etsy-only override başlıkları AYNI anahtar şemasıyla reçete satırına
/// sahip olabilir). Net/tutar CANLI hesaplanır (<c>ProductRecipeCostCalculator</c> AYNEN yeniden kullanılır — saf/DB'siz).
/// Ürün reçetesiyle AYNI alan seti — hesap motoru ortak kalsın diye birebir hizalı. N11 <c>SalesChannelTrN11ProductStockItemRecipeLine</c>
/// ikizi AYNEN.
/// </summary>
public class SalesChannelEtsyProductStockItemRecipeLine : FullAuditedAggregateRoot<Guid>, IMultiTenant, ICompanyOwned
{
    #region Constructors

    protected SalesChannelEtsyProductStockItemRecipeLine()
    {
    }

    public SalesChannelEtsyProductStockItemRecipeLine(
        Guid companyId,
    ...
    /// <summary>Reçetenin ait olduğu override başlığının (<see cref="SalesChannelEtsyProductStockItem"/>) KENDİ id'si —
    /// id-only referans, set-once. ERP ProductVariantId DEĞİL — böylece ProductVariantId'si null olan Etsy-only
    /// kombinasyon satırları da reçete taşıyabilir.</summary>
